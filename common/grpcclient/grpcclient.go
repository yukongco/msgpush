...
// New grpc pool
func NewPoolTimeout(addres string, init, capacity int, idleTimeout time.Duration) *Pool {
	pool, err := New(func() (*grpc.ClientConn, error) {

		clientConn, err := grpc.Dial(addres, grpc.WithInsecure())
		if err != nil {
			fmt.Println("grpc dial err: %v")
	...
	if err != nil {
		fmt.Println("init client pool err: ", err)
		return nil
	}
	return pool
}
...
func (p *Pool) Get(ctx context.Context) (*ClientConn, error) {
	...
	if wrapper.Client != nil && idleTimeout > 0 &&
